...
def plot_board_scores(root_id, state_tree, distinct_actions):
    q_values = np.zeros(64)
    for action in state_tree[root_id]["child"]:
        child_id = root_id + (action,)
        q = state_tree[child_id]["q"]
        q_values[action] = q
    q_values = q_values.reshape(8, 8)
    print(q_values.reshape(8, 8))
    plot_score_grid(q_values)


def select_action(game_state):
    root_id = (0,)
    distinct_actions = game_state.num_distinct_actions()
    ...
    plot_board_scores(root_id, state_tree, distinct_actions)
